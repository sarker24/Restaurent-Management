...
If cboMode.text = "Guest Card No" Then
      ...
      rsTemp.Open "SELECT CID,DCard,CName,CAddress,Cphone " & _
                 "FROM RMSCustomer WHERE RMSCustomer.Dcard LIKE '" & txtSearch.text & "%'", cn, adOpenStatic, adLockReadOnly
        
ElseIf cboMode.text = "Guest Name" Then

      rsTemp.Open "SELECT CID,DCard,CName,CAddress,Cphone " & _
                 "FROM RMSCustomer WHERE RMSCustomer.CName Like '" & txtSearch.text & "%'", cn, adOpenStatic, adLockReadOnly

'Else cboMode.text = "Guest Phone No" Then
Else

      rsTemp.Open "SELECT CID,DCard,CName,CAddress,Cphone " & _
                 "FROM RMSCustomer WHERE RMSCustomer.CPhone LIKE '" & txtSearch.text & "%'", cn, adOpenStatic, adLockReadOnly
...
fgExport.Rows = 1
    ...
    fgExport.AddItem "" & vbTab & rsTemp("CID") & vbTab & rsTemp("DCard") & _
         vbTab & rsTemp("CName") & vbTab & rsTemp("CAddress") & vbTab & rsTemp("Cphone")
   
'        fgExport.AddItem "" & vbTab & rsTemp("CID") & vbTab & rsTemp("DCard")& vbTab & rsTemp("CName")& vbTab & rsTemp("C")
...
Private Sub Form_Load()
     ...
     rsTemp.Open "SELECT CID,DCard,CName,CAddress,CPhone,BDate, " & _
                 "CEmail,DiscountAmt,RPoint FROM RMSCustomer", cn, adOpenStatic, adLockReadOnly
        
            fgExport.Rows = 1
    
    While Not rsTemp.EOF
        fgExport.AddItem "" & vbTab & rsTemp("CID") & vbTab & rsTemp("Dcard") & _
         vbTab & rsTemp("CName") & vbTab & rsTemp("CAddress") & _
         vbTab & rsTemp("CPhone") & vbTab & rsTemp("BDate") & vbTab & rsTemp("CEmail")
         
        rsTemp.MoveNext
    Wend
     GridCount fgExport
     
        cboMode.AddItem "Guest Card No"
       cboMode.AddItem "Guest Name"
       cboMode.AddItem "Guest Mobile No"
